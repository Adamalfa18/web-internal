...
document.getElementById('searchInput').addEventListener('input', function () {
    ...
    document.querySelectorAll('tbody tr').forEach((row) => {
        ...
        row.style.display = (clientName.includes(searchValue) || brandName.includes(searchValue)) ? '' : 'none';
    });
});
...
document.querySelectorAll('input[name="btnradiotable"]').forEach((radio) => {
    radio.addEventListener('change', function () {
        ...
        document.querySelectorAll('.client-item').forEach((item) => {
            ...
            item.style.display = (itemStatus == selectedValue || (selectedValue == 2 && itemStatus !== '1')) ? 'block' : 'none';
        });
    });
});
